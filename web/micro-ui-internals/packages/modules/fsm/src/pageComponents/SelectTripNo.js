...
const SelectTripNo = ({ config, formData, t, onSelect, userType }) => {
  ...
  const { data: tripNumberData, isLoading } = Digit.Hooks.fsm.useMDMS(
    stateId,
    "FSM",
    "TripNumber"
  );
  ...
  useEffect(() => {
    if (!isLoading && tripNumberData) {
      const preFilledTripNumber = tripNumberData.filter(
        (tripNumber) =>
          tripNumber.code ===
          (formData?.selectTripNo?.tripNo?.code || formData?.selectTripNo)
      )[0];
      preFilledTripNumber
        ? setTripNo(preFilledTripNumber)
        : setTripNo(tripNumberData.find((i) => i.code === 1));
    }
  }, [formData?.selectTripNo?.tripNo, tripNumberData]);

  useEffect(() => {
    if (!isLoading && vehicleMenu) {
      const preFilledCapacity = vehicleMenu.filter(
        (i) => i.capacity === formData?.selectTripNo?.vehicleCapacity?.capacity
      )[0];
      ...
      preFilledCapacity
        ? setVehicleCapacity(preFilledCapacity)
        : setVehicleCapacity(minCapacity);
    }
